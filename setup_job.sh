...
if ! java -jar /var/jenkins_home/jenkins-cli.jar -s http://172.17.0.1:8080/ -auth admin:admin get-job triggerpythonjob ; then 
    java -jar /var/jenkins_home/jenkins-cli.jar -s http://172.17.0.1:8080/ -auth admin:admin create-job triggerpythonjob < /var/jenkins_home/jobs/triggerpythonjob/config.xml && echo "Job created"
fi
