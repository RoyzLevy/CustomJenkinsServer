...
if ! java -jar /usr/local/bin/jenkins-cli.jar -s http://localhost:8080/ -auth admin:admin get-job triggerpythonjob ; then 
    java -jar /usr/local/bin/jenkins-cli.jar -s http://localhost:8080/ -auth admin:admin create-job triggerpythonjob < /var/jenkins_home/jobs/triggerpythonjob/config.xml && echo "Job created"
fi
